...
def _is_off_mode(hvac_mode) -> bool:
    """Return True when the HVAC mode should disable the actuator."""
...
def compute_on_percent(
    *,
    ...
    """Compute the nominal clamped actuator command."""
    if _is_off_mode(hvac_mode):
        return 0.0

    e_in = mode_sign * (target_temp - current_temp)
    e_out = mode_sign * (target_temp - outdoor_temp) if outdoor_temp is not None else 0.0
